...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: "projects", label: "Proyectos", icon: <FolderKanban size={20} /> },
  { id: "providers", label: "Proveedores", icon: <Users size={20} /> },
  { id: "accounts", label: "Cuentas", icon: <Wallet size={20} />, requiresFull: true },
  { id: "personal", label: "Finanzas Personal", icon: <User size={20} />, requiresFull: true },
  { id: "nitia-costs", label: "Costos Nitia", icon: <Building2 size={20} />, requiresFull: true },
  { id: "settings", label: "Configuración", icon: <Settings size={20} />, requiresFull: true },
]

export function AppSidebar() {
  const { role, setRole, section, setSection, setSelectedProjectId } = useApp()
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          if (item.requiresFull && !isFull) return null
