...
export async function validatePinLogin(pin: string) {
  try {
    ...
    const { data: user, error } = await supabase
      .from("users")
      .select("id, role, pin, totp_enabled, totp_secret")
      .eq("pin", pin)
      .single()
...
export async function createEmployeeUser(name: string, pin: string) {
  try {
    ...
    if (error) {
      if (error.message.includes("empleada")) {
        return { success: false, error: "Ya existe una empleada. Edítala o elimínala primero." }
      }
      console.error("Create employee error:", error)
      return { success: false, error: "Error al crear empleada" }
    }

    return { success: true, data: data?.[0], message: "Empleada creada exitosamente" }
  } catch (error) {
    console.error("Create employee error:", error)
    return { success: false, error: "Error al procesar solicitud" }
  }
...
export async function updateEmployeeUser(name: string, pin: string) {
  try {
    if (pin.length !== 4 || !/^\d+$/.test(pin)) {
      return { success: false, error: "PIN debe ser 4 dígitos numéricos" }
    }
    ...
    const supabase = getSupabaseClient()
    // Buscar y actualizar empleada
    const { data, error } = await supabase
      .from("users")
      .update({ name, pin, updated_at: new Date().toISOString() })
      .eq("role", "empleada")
      .select()

    if (error) {
      console.error("Update employee error:", error)
      return { success: false, error: "Error al actualizar empleada" }
    }

    return { success: true, data: data?.[0], message: "Empleada actualizada exitosamente" }
  } catch (error) {
    console.error("Update employee error:", error)
    return { success: false, error: "Error al procesar solicitud" }
  }
...
export async function requestPinChangeEmail(email: string) {
  try {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || "https://v0-deploy-nitia-estudio.vercel.app"}/reset-pin?email=${encodeURIComponent(email)}`,
    })
