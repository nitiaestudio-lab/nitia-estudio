...
function AppContent() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F7F5ED]">
      ...
      {/* Floating Action Button - Add Movement */}
      {isFull && (
        <button
          onClick={() => setShowGlobalMovement(true)}
          className="fixed bottom-6 right-6 z-30 w-14 h-14 bg-[#5F5A46] text-white rounded-full shadow-xl shadow-[#5F5A46]/30 flex items-center justify-center hover:bg-[#4A4639] transition-all hover:scale-105 active:scale-95"
      ...
      {showGlobalMovement && (
        <GlobalMovementModal onClose={() => setShowGlobalMovement(false)} />
      )}
    </div>
  )
}
